...
_Bool releaseDoor(void);

_Bool releaseDoor1(void);

_Bool releaseDoor2(void);

_Bool depositBall(void);
...
typedef struct{
    unsigned char* _dataTX;
    unsigned char _numBytesTX;
    volatile unsigned char _numTransmits;
    unsigned char* _dataRX;
    unsigned char _numBytesRX;
    volatile unsigned char _numReceives;
    volatile unsigned char _stateTX;
    volatile unsigned char _stateRX;
    volatile unsigned char _lastReceiveFERR;
    volatile unsigned char _lastReceiveOERR;
}UART_t;
...
void main(void) {
    ...
    analog1Ambient = distFromCanister(1);
    analog2Ambient = distFromCanister(2);
    analog3Ambient = distFromCanister(7);
    analog4Ambient = distFromCanister(8);
    initStartScreen();
...
# 258 "main.c"
    ...
    dist2 = distFromCanister(2);
    dist3 = distFromCanister(7);
    dist4 = distFromCanister(8);
    if (dist2 > 0 )
    {
        while ((dist3 > 975)|| (dist3 < 975))
        {
            if ((dist3 > 975))
                moveRobot(3);
            else if ((dist3 < 975))
    ...
    dist1 = distFromCanister(1);

    dist2 = distFromCanister(2);

    dist3 = distFromCanister(7);
    dist4 = distFromCanister(8);
    if ((dist1 > 700)&&(dist2 > 500)&&(dist4 < 750))
    {
        state = 1;
    }
    else
...
# 355 "main.c"
    ...
    storeCanisterData(facing, location, status, deposited);
# 451 "main.c"
    while(readDigitalSensor())
    {
        moveRobot(1);
...
void testTurn(int count)
{
    ...
    int newCount = shaftCount;
    unsigned int shaftCount2 = shaftEncoderCount2();
    while ((shaftCount - newCount) <= count)
    {
        moveRobot(5);
...
void dispSensors2(void)
{
    ...
    int newDist;
    int digDist;
    oldDist = distFromCanister(1);
    newDist = distFromCanister(2);


    digDist = readDigitalSensor();
    sensorData(oldDist, newDist);
    { lcdInst(0x80 | LCD_LINE3_ADDR);};
    printf("Digital: %d", digDist);
    _delay((unsigned long)((1000)*(10000000/4000.0)));
    { lcdInst(0x01); _delay((unsigned long)((5)*(10000000/4000.0)));};
}
...
{
    int dist1, dist2;
    long int oldDigDist, newDigDist;
    float avgDist1 = distFromCanister(1);
    float avgDist2 = distFromCanister(2);
    float startTime = getTime();
    oldDigDist = readDigitalSensor();
    while (1)
    {
        newDigDist = readDigitalSensor();
        if (newDigDist != oldDigDist)
        {
            break;
        }
        if ((getTime() - startTime) > 0.1)
        {
            dist1 = distFromCanister(1);
            dist2 = distFromCanister(2);
            avgDist1 = (avgDist1 + dist1)/(2.0);
            avgDist2 = (avgDist2 + dist2)/(2.0);
            startTime = getTime();
            { lcdInst(0x01); _delay((unsigned long)((5)*(10000000/4000.0)));};
            { lcdInst(0x80); _delay((unsigned long)((2)*(10000000/4000.0)));}
            printf("Dist1: %.2f", avgDist1);
            { lcdInst(0x80 | LCD_LINE2_ADDR);};
            printf("Dist2: %.2f", avgDist2);
        }
    }
...
void adjustCanister(void)
{
    int dist1 = distFromCanister(1);
    int dist2;
    int diff;
    float ratio;
    while (dist1 > 650 || (readDigitalSensor()))
    {
    ...
    if (dist2 > dist1)
    {
        while(1)
        {
            ...
            { lcdInst(0x80 | LCD_LINE3_ADDR);};
            printf("Dist2: %d", dist2);
            { lcdInst(0x80 | LCD_LINE4_ADDR);};
            printf("Digital: %d", readDigitalSensor());
